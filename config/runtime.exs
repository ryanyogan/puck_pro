...
if config_env() in [:dev, :test] do
  Dotenvy.source!([".env", ".env.#{config_env()}"],
    side_effect: fn envs ->
      Enum.each(envs, fn {key, val} -> System.put_env(key, val) end)
    end
  )
end
...
#
#     PHX_SERVER=true bin/puck_pro start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :puck_pro, PuckProWeb.Endpoint, server: true
end

# Anthropic API key for AI features
if api_key = System.get_env("ANTHROPIC_API_KEY") do
  config :puck_pro, anthropic_api_key: api_key
end
...
if config_env() == :prod do
  database_path =
    System.get_env("DATABASE_PATH") ||
      ...
      For example: /etc/puck_pro/puck_pro.db
      """

  config :puck_pro, PuckPro.Repo,
  ...
  config :puck_pro, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
